...
alias git-svn-dcommit-push='git vn dcommit && git push github master:svntrunk'
...
alias gwip='git add -A; git rm $(git ls-files --deleted) 2> /dev/null; git commit --no-verify -m "--wip-- [skip ci]"'s
